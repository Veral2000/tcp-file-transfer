...
std::uint32_t read_u32(const std::vector<std::uint8_t>& data, std::size_t& pos) {
    if (pos > data.size() || data.size() - pos < 4U) throw std::runtime_error("truncated uint32");
    std::uint32_t value = 0;
    for (int i = 0; i < 4; ++i) value = (value << 8U) | data[pos + static_cast<std::size_t>(i)];
    pos += 4U;
    return value;
}
std::uint64_t read_u64(const std::vector<std::uint8_t>& data, std::size_t& pos) {
    if (pos > data.size() || data.size() - pos < 8U) throw std::runtime_error("truncated uint64");
    std::uint64_t value = 0;
    for (int i = 0; i < 8; ++i) value = (value << 8U) | data[pos + static_cast<std::size_t>(i)];
    pos += 8U;
    return value;
}
std::vector<std::uint8_t> encode_header(MessageType type, std::uint64_t payload_size) {
    ...
    append_u32(header, kMagic); append_u16(header, kVersion);
    header.push_back(static_cast<std::uint8_t>(type)); header.push_back(0U);
...
void send_message(network::TcpSocket& socket, MessageType type, const std::vector<std::uint8_t>& payload) {
    if (payload.size() > kMaxPayloadSize) throw std::runtime_error("payload exceeds protocol maximum");
...
MessageHeader receive_header(network::TcpSocket& socket) {
    ...
    std::size_t pos = 0;
    const auto magic = read_u32(header, pos);
    const auto version = read_u16(header, pos);
    const auto type = static_cast<MessageType>(header[pos++]);
    ++pos;
    const auto payload_size = read_u64(header, pos);
    ...
    switch (type) {
        case MessageType::Hello: case MessageType::FileInfo: case MessageType::Chunk:
        case MessageType::TransferComplete: case MessageType::Error: case MessageType::FileHash: break;
        default: throw std::runtime_error("unknown message type");
    }
    return MessageHeader{type, payload_size};
}

std::vector<std::uint8_t> receive_payload(network::TcpSocket& socket, const MessageHeader& header) {
...
void send_hello(network::TcpSocket& socket) { send_message(socket, MessageType::Hello, {}); }

void send_file_info(network::TcpSocket& socket, const FileInfo& info) {
    if (info.filename.empty() || info.filename.size() > kMaxFilenameLength) throw std::runtime_error("invalid filename length");
    if (info.size > kMaxFileSize) throw std::runtime_error("file exceeds 16 GiB limit");
...
void send_file_hash(network::TcpSocket& socket, const crypto::Sha256Digest& digest) {
    send_message(socket, MessageType::FileHash, std::vector<std::uint8_t>(digest.begin(), digest.end()));
}

void send_chunk(network::TcpSocket& socket, const Chunk& chunk) {
    if (chunk.data.empty() || chunk.data.size() > kMaxChunkSize) throw std::runtime_error("invalid chunk size");
    std::vector<std::uint8_t> payload;
    payload.reserve(20U + chunk.data.size());
    append_u64(payload, chunk.index); append_u64(payload, chunk.offset);
...
void send_transfer_complete(network::TcpSocket& socket) { send_message(socket, MessageType::TransferComplete, {}); }
...
void send_error(network::TcpSocket& socket, const std::string& message) {
    ...
    send_message(socket, MessageType::Error, std::vector<std::uint8_t>(message.begin(), message.end()));
}

FileInfo parse_file_info(const std::vector<std::uint8_t>& payload) {
    std::size_t pos = 0;
    const auto size = read_u64(payload, pos);
    const auto name_length = read_u16(payload, pos);
    if (name_length == 0U || name_length > kMaxFilenameLength || payload.size() - pos != name_length) throw std::runtime_error("invalid file metadata");
...
crypto::Sha256Digest parse_file_hash(const std::vector<std::uint8_t>& payload) {
    if (payload.size() != 32U) throw std::runtime_error("invalid SHA-256 digest length");
...
Chunk parse_chunk(const std::vector<std::uint8_t>& payload) {
    std::size_t pos = 0;
    const auto index = read_u64(payload, pos);
    const auto offset = read_u64(payload, pos);
    const auto data_size = read_u32(payload, pos);
    if (data_size == 0U || data_size > kMaxChunkSize || payload.size() - pos != data_size) throw std::runtime_error("invalid chunk payload");
...
std::string parse_error(const std::vector<std::uint8_t>& payload) { return std::string(payload.begin(), payload.end()); }
